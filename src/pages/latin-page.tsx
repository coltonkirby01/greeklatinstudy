...
const latinCardFitCss = `
/* Henle adjective and participle answer sides size themselves to their content.
   Both sides share one intrinsic CSS-grid row, so even narrow screens do not
   need an inner flashcard scrollbar or risk clipped Latin forms. */
.flashcard-scene:has(.latin-card-auto-fit) {
  min-height: 370px;
}
.flashcard-scene:has(.latin-card-auto-fit) .flashcard-inner {
  display: grid;
  height: auto;
  min-height: 370px;
}
.flashcard-scene:has(.latin-card-auto-fit) .flashcard-face {
  position: relative;
  inset: auto;
  grid-area: 1 / 1;
  min-height: 370px;
  height: auto;
  align-self: stretch;
  padding: clamp(3.15rem, 5vw, 3.7rem) clamp(.7rem, 2.7vw, 1.4rem) clamp(.8rem, 2vw, 1.1rem);
  overflow: visible;
}
.latin-participle-answer {
  display: grid;
  width: 100%;
  max-width: 43rem;
...
.latin-participle-answer .participle-answer-row {
  display: grid;
...
.latin-participle-answer .participle-answer-label {
  color: var(--burgundy);
  font-weight: 750;
}
.latin-participle-answer .participle-answer-value {
  min-width: 0;
  font-weight: 550;
  overflow-wrap: break-word;
}
.latin-participle-answer .participle-answer-example {
  display: flex;
  flex-wrap: wrap;
  gap: .45rem;
  margin-top: .15rem;
...
.latin-adjective-answer {
  width: 100%;
  max-width: 760px;
  gap: clamp(.3rem, .9vw, .5rem);
}
.latin-adjective-answer .henle-card-title {
  font-size: clamp(1rem, 2.1vw, 1.36rem);
  line-height: 1.16;
}
.latin-adjective-answer .answer-notes {
  margin: 0;
  font-size: clamp(.81rem, 1.7vw, .98rem);
  line-height: 1.2;
}
.latin-adjective-answer .answer-notes:first-of-type {
  color: var(--burgundy);
  font-weight: 750;
}
.flashcard-scene:has(.latin-adjective-answer) .flashcard-face .henle-chart th,
.flashcard-scene:has(.latin-adjective-answer) .flashcard-face .henle-chart td {
...
@media (max-width: 480px) {
  .latin-participle-answer .participle-answer-row {
    ...
    gap: .13rem;
    font-size: clamp(.96rem, 3.5vw, 1.09rem);
    line-height: 1.26;
  }
  .latin-participle-answer {
    gap: .55rem;
...
function ParticipleDeckFilters({
  deck,
  active,
  selection,
  isExcluded,
  ...
  const material: Material = "participles";
  const ids = deck?.cards.map((card) => card.id) ?? [];
  const selectedCount = deck?.cards.filter((card) =>
    active && selected(selection, card.id) && !isExcluded(deck.id, card.id)
  ).length ?? 0;
  const checked = Boolean(ids.length && selectedCount === ids.length);
  const mixed = selectedCount > 0 && selectedCount < ids.length;

  return (
    <FilterDisclosure
      title="Participles"
      count={deck?.cards.length ?? 4}
      summary={`${selectedCount} of ${ids.length || 4} selected`}
      nested
      checked={checked}
      mixed={mixed}
      onCheckedChange={(selectAll) => onDeckChange(material, ids, selectAll)}
    >
      {deck && <FilterSection title="Participles">
        {deck.cards.map((card) => (
          <FilterCheckbox
            key={card.id}
            label={card.front}
...
export function LatinPage() {
  ...
  const sources = useMemo(() => {
    ...
    if (participlesActive && participlesDeck && participleStudyCards.length) {
      next.push({ id: "participles", label: "Participles", deck: participlesDeck, cards: participleStudyCards, studyKey: "forward", direction: "forward" });
    }
    ...
    if (includeSavedCards) {
      ...
      if (participlesDeck) {
        const cards = participlesDeck.cards.filter((card) => {
          const ref = savedCardRef(participlesDeck.id, card.id);
          return savedCards.refs.has(ref) && !excludedCards.refs.has(ref) && !alreadySelected.has(ref);
        });
        if (cards.length) {
          next.push({ id: "saved-participles", label: "Saved Cards", deck: participlesDeck, cards, studyKey: "forward", direction: "forward" });
          cards.forEach((card) => alreadySelected.add(savedCardRef(participlesDeck.id, card.id)));
        }
      }
  ...
  const virtualDeck = useMemo<DeckDefinition>(() => ({
    ...
    description: "A unified Latin study surface for Dickinson vocabulary and selected Henle grammar cards.",
    language: "latin",
    cards: selectedCards,
    ...
    <main className="page-shell study-page latin-page">
      ...
      >
        ...
        >
          ...
          />
          <ParticipleDeckFilters
            deck={participlesDeck}
            active={participlesActive}
            selection={paradigmCards}
            isExcluded={excludedCards.isExcluded}
            onRestore={excludedCards.restore}
            onExclude={excludedCards.exclude}
