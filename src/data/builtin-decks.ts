...
type GreekVocabularySourceCard = { id: string; greek: string; meaning: string; part_of_speech: string; lesson: number };
type GreekGrammarChartRow = { label: string; cells: string[] };
type GreekGrammarSourceCard = { id: string; category: string; prompt: string; columns: string[]; rows: GreekGrammarChartRow[] };
  ...
  greekLesson3VocabularyPromise ??= fetchText("data/greek-lesson3-vocab.json").then((text) => {
    ...
    const cards: StudyCard[] = source.map((card, index) => {
      ...
      return {
        ...
        metadata: { lesson: card.lesson, studySource: "vocabulary", partOfSpeech: card.part_of_speech, pronunciation, sourceRef: "Groton 3.24" },
      };
    });
    return { id: "alpha-omega-lesson3-vocab", slug: "greek", title: "Greek Lesson 3 Vocabulary", eyebrow: "Lesson 3 vocabulary", description: "Eleven Lesson 3 vocabulary entries.", language: "greek", cards, supportsReverse: true, sourceNote: "Groton 3.24." } satisfies DeckDefinition;
  });
    ...
    const cards: StudyCard[] = source.map((card, index) => {
      ...
      return {
        ...
        metadata: {
          ...
          sourceRef: lesson3GrammarSourceRef(card.category),
        },
      };
    });
    return { id: "alpha-omega-lesson3-grammar", slug: "greek", title: "Greek Lesson 3 Grammar", eyebrow: "Present active paradigms", description: "Three whole-paradigm chart cards.", language: "greek", cards, supportsReverse: false, sourceNote: "Groton 3.20–3.22." } satisfies DeckDefinition;
  });
  ...
  greekLesson4VocabularyPromise ??= fetchText("data/greek-lesson4-vocab.json").then((text) => {
    ...
    const cards: StudyCard[] = source.map((card, index) => {
      ...
      return {
        ...
        notes: `${card.part_of_speech} · Pronunciation: ${pronunciation}`,
        metadata: { lesson: card.lesson, studySource: "vocabulary", partOfSpeech: card.part_of_speech, pronunciation, sourceRef: "Groton 4.32" },
      };
    });
    return { id: "alpha-omega-lesson4-vocab", slug: "greek", title: "Greek Lesson 4 Vocabulary", eyebrow: "Lesson 4 vocabulary", description: "Eleven Lesson 4 vocabulary entries.", language: "greek", cards, supportsReverse: true, sourceNote: "Groton 4.32." } satisfies DeckDefinition;
  });
...
export function loadGreekLesson4GrammarDeck() {
  greekLesson4GrammarPromise ??= fetchText("data/greek-lesson4-grammar.json", "no-store").then((text) => {
    ...
    const cards: StudyCard[] = source.map((card, index) => {
      ...
      const sourceRef = card.id.includes("definite-article") ? "Groton 4.30" : "Groton 4.29";
      return {
        id: card.id,
        ...
        rank: index + 1,
        source: "From Alpha to Omega, Lesson 4",
        notes: card.id.includes("definite-article") ? "Feminine definite article" : "Whole-paradigm first-declension feminine noun",
        metadata: {
          lesson: 4,
          studySource: "grammar-chart",
          grammarGroup: card.category,
    ...
    return { id: "alpha-omega-lesson4-grammar", slug: "greek", title: "Greek Lesson 4 Grammar", eyebrow: "First-declension feminine nouns · article", description: "Six whole-paradigm chart cards.", language: "greek", cards, supportsReverse: false, sourceNote: "Groton 4.29–4.30." } satisfies DeckDefinition;
  });
