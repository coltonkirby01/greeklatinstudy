...
export function jsonToCards(value: unknown) {
  ...
  return source.map((item, index) => {
    ...
    const pronunciation = card["Pronunciation Text"] ?? card.pronunciationText ?? card.audioText;
    if (typeof pronunciation === "string" && pronunciation.trim()) metadata.pronunciationText = pronunciation.trim();
    return { front: String(card.Front ?? card.front ?? card.prompt ?? "").trim(), back: String(card.Back ?? card.back ?? card.answer ?? "").trim(), category: String(card.Category ?? card.category ?? "").trim(), rank: Number(card.Rank ?? card.rank ?? index + 1), source: String(card.Source ?? card.source ?? "").trim(), notes: String(card.Notes ?? card.notes ?? card.note ?? "").trim(), reversePrompt: String(card["Reverse Prompt"] ?? card.reversePrompt ?? card.reverse_prompt ?? "").trim(), metadata: Object.keys(metadata).length ? metadata : undefined };
