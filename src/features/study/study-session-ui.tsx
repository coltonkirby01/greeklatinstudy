...
export function StudySidebar({ copy, direction, stats, sessionId, initialProgress, progressScope = "session", priority, priorityPrompt, cardCopy }: {
  copy: DirectionalCardCopy;
  direction: StudyDirection;
  stats: StudyStats;
  sessionId?: string;
  initialProgress?: InitialProgress;
  progressScope?: "session" | "visit";
  priority: Priority;
  ...
  const visit = progressScope === "visit";
  const progressLabel = completionDone ? (visit ? "Visit mastery" : "Initial mastery") : (visit ? "Visit completion" : "Initial completion");
  const progressAria = completionDone ? `${visit ? "Page-visit" : "Initial"} mastery of selected cards` : `${visit ? "Page-visit" : "Initial"} completion of selected cards`;
  const progressNote = completionDone ? `of selected cards answered Right at least once ${visit ? "this page visit" : "this session"}` : `of selected cards reviewed ${visit ? "this page visit" : "this session"}`;
  return <aside className="study-sidebar">
    <style>{sharedStudyPolishCss}</style>
    <section className="panel-surface stats-panel">
      <div className="sidebar-heading"><div><p className="eyebrow">{visit ? "Current page visit" : "Current session"}</p><h2>Progress · {copy.sideLabel}</h2></div><Gauge /></div>
