...
- Every built-in card must also be individually selectable from the language-level Choose cards menu. Exact-card selection and the answer-side Deselect action use the same persistent per-card exclusion state; they are not parallel selection systems. There is no separate Individually deselected list.
...
- Filters narrow the current study pool only. Deselecting a source, child, or individual card must never erase or reset stored mastery, history, scheduling, timing, or statistics for those cards.
...
- Changing a study filter or direction returns the active study surface to the Start gate before timing resumes. The answer-side Deselect action is the exception: it is deferred until Save & Next and must not reopen the Start gate when the next card appears.
...
- Current Greek vocabulary sources are Lessons 3, 4, and 5 Vocabulary.
...
- Greek card types remain multi-select. Lesson material, vocabulary, grammar, punctuation, accents, and future lesson categories may be combined in one study pool without merging their stored histories.
...
- Latin vocabulary and Latin grammar/paradigm sources can be selected singly or combined in one study pool.
...
- Henle adjective data must be fetched fresh when the six-card source expands: a stale cached four-card JSON must never silently hide the newer R. 80 and R. 82 cards. Older selections containing R. 78 are migrated once to include the two new sibling cards; subsequent manual deselections must remain respected. Latin adjective and participle answer faces must grow with their content rather than introduce an inner scrollbar, and both Formation and Declension on participle answers must have matching typography.
- Henle third-declension adjective cards are grouped inside the existing Adjectives selector: R. 78 (`gravis, -e`) is **two terminations**, R. 80 (`ācer, ācris, ācre`) is **three terminations**, and R. 82 (`dīligēns, dīligentis`) is **one termination**. Show the applicable phrase explicitly on the answer side, not just the selector, and preserve the complete R80/R82 charts and contrasting neuter forms.
- The stable adjective card IDs are `latin-adjective-1st-2nd-masculine`, `latin-adjective-1st-2nd-feminine`, `latin-adjective-1st-2nd-neuter`, `latin-adjective-3rd-gravis`, `latin-adjective-3rd-acer`, and `latin-adjective-3rd-diligens`. Do not merge, split, or rename them without an explicit content migration.
- `latin-participles` is a separate built-in four-card deck under Grammar (Henle) → Participles. Its four stable cards teach Present Active, Perfect Passive, Future Active, and Future Passive participle formation and declension, in that order. Present Active (which declines as a third-declension adjective **of one termination**) and Future Passive use the present stem derived from the **second** principal part; Perfect Passive and Future Active use the **fourth** principal part. Future Active uses `-ūrus, -ūra, -ūrum`; Future Passive uses `-ndus, -nda, -ndum` and is also called the gerundive. Preserve conjugation-specific connecting vowels, the correct Latin examples, shared exact-card selection/Deselect/Shuffle, Saved Cards, and Stats. These are textual formation cards, not audio paradigms; do not automatically generate TTS for them.
...
- For *From Alpha to Omega*, preserve stable existing vocabulary IDs when later lessons add principal parts to an already introduced verb. Lessons 6–7 are registered built-in Greek sources: Lesson 6 introduces future active forms, second principal parts, and the p. 32 Letter Changes chart; Lesson 7 introduces second-declension masculine nouns and the masculine definite article. New Groton cards must follow the existing Greek source-ref, accent, chart, exact-selection, Deselect, Shuffle, Stats, and session conventions. When correcting Groton vocabulary, check every entry against the fourth-edition lesson and glossary, preserve existing card IDs and review histories, and update the corresponding future audio source without triggering paid TTS. In §7.47, `lesson7-v8` is **λῡ́πη, -ης, ἡ** (“pain, grief”), not πόνος; the 12-entry audit is protected by `tests/greek-lessons-6-7.test.ts`.
...
- Adding cards to an existing registered deck must automatically reach Stats and session coverage because the catalog loads the complete deck. New built-in cards must also inherit exact-card selection, Shuffle, and the shared answer-side Deselect control without card-specific wiring.
- If a new lesson/category is introduced, extend the language selector so Select all/Deselect all and saved filter preferences handle it safely.
...
- Built-in source counts are protected by regression tests. Deliberately update those tests only when authoritative source material is intentionally expanded.
- Keep existing active card IDs stable; retired IDs may remain only in historical progress storage and must never silently re-enter active study pools.
...
- `studyKey` separates Forward, Reverse, and specialized study modes. Never merge their mastery, statistics, review history, or scheduling state.
...
- Highest-Priority Review remains long-term/adaptive and may use the user's continuous learning history for scoring, but its candidate list MUST be limited to the currently selected/available card pool. Do not make the list page-visit-only, and do not allow deselected cards into it.

## Sessions and warm-ups

- A persistent study session is a Stats/organization window layered on top of continuous long-term mastery. Starting a new session must never reset mastery, due dates, intervals, response-time history, or adaptive priorities.
...
- Both Greek and Latin study toolbars expose one compact Session dropdown. That menu lets the user keep the current persistent session, start a new session, or select a resumable past session.
- Toolbar select controls (including Adaptive/Sequential/Shuffle and Session) use a clean, fully visible dropdown indicator with enough right-side padding; never let the arrow crowd or clip against the rounded edge.
...
- A study session begins behind an explicit Start gate. The timer remains at rest until the user presses Start or Space as implemented by the shared controller.
...
- Toolbar controls and Start-gate buttons retain normal keyboard behavior while the gate is open; interacting with them must not be misinterpreted as the global start gesture.
...
- Marking a visible answer-side card with D does not require or trigger a Start gate; the marked exclusion is applied only on the following Save & Next transition.
...
- Space starts while the Start gate is open, reveals an unrevealed card during active study, and performs Save & Next after reveal.
...
- D = Deselect card on the visible answer side. It toggles a pending visual `Deselected` state only; it must not immediately mutate the active pool, change cards, or reopen the Start gate. The actual individual exclusion is committed when Space performs Save & Next.
- S saves or unsaves the current card where Saved Cards is supported. A controls card audio where audio is available.
...
- Deselecting a source, lesson, part of speech, grammar category, direction, or individual card must never delete that source's stored progress.
- Per-card exclusion is a selection preference, not learning-state deletion. Exact-card checkboxes and answer-side D must share the same exclusion state.
...
- Stats retain Forward/Reverse separation and include specialized study modes as their own study modes where configured.
- Stats rank explicit persistent study sessions. Legacy history without explicit session IDs may be grouped into inferred sessions without altering stored data.
...
- Henle intrinsic difficulty follows rule progression plus grammatical complexity encoded in section/subsection, voice, mood/form, special/irregular families, and whole-chart reconstruction where those sources are active.
