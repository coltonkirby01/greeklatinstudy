...
- Never cache an obsolete four-card adjective JSON when six cards exist. Keep a freshness guard and a one-time migration for saved legacy third-declension selections; do not silently override a later intentional per-card deselection. Answer faces for adjectives and participles must fit their complete text without an inner scrollbar, with responsive intrinsic sizing. Present participle Formation and Declension lines use identical typography.
